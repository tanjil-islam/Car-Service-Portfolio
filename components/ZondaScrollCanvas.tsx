...
export default function ZondaScrollCanvas({
  ...
  return (
    <div className="relative w-full h-full bg-pagani-black flex items-center justify-center">
      {/* CANVAS */}
      <canvas
        ref={canvasRef}
      ...
      {/* LOADER */}
      {!isLoaded && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-pagani-black z-20">
          <div className="w-64 border border-pagani-gold/20 p-1 rounded-sm overflow-hidden mb-4">
            <div
              className="h-1.5 bg-pagani-gold transition-all duration-300"
              style={{ width: `${loadingProgress}%` }}
            />
          </div>
          <span className="font-orbitron text-xs tracking-[0.3em] text-pagani-gold">
            LOADING TELEMETRY // {loadingProgress}%
          </span>
        </div>
      )}

      {/* SUBTLE GLOW OVERLAY */}
      <div className="pointer-events-none absolute inset-0 bg-radial-gradient from-transparent via-[#1a1a1ade]/50 to-[#1a1a1a]" />
    </div>
  );
}
